...
**Data:** 13/05/2026, 03:41:29
**Duração total:** 11633ms
...
| T30 | latência de todas as tools < 500ms com SQLite local | ✓ pass | 0ms |  |
...
### t04-ftp (0 passaram / 6 falharam / 9 total)

| ID | Nome | Status | Duração | Detalhe |
|----|------|--------|---------|---------|
| FTP01 | listarArquivos ST SP retorna lista não-vazia | ✗ fail | 2388ms | AssertionError: FTP deve retornar arquivos ST/SP: expected 0 > 0 |
| FTP02 | listarArquivos LT SC retorna arquivos | ✗ fail | 2375ms | AssertionError: assertGreater: expected 0 > 0 |
| FTP03 | listarArquivos ordena por competência mais recente primeiro | ✗ fail | 2233ms | AssertionError: precisa de ao menos 2 arquivos para comparar ordem: expected 0 > 1 |
| FTP04 | listarArquivos nome segue padrão GRUPO+UF+AAAA+MM.dbc | ✗ fail | 331ms | AssertionError: assertGreater: expected 0 > 0 |
| FTP05 | listarArquivos PF SP tem competência recente (após 2024) | ✗ fail | 363ms | AssertionError: assertGreater: expected 0 > 0 |
| FTP06 | downloadArquivo ST SC baixa arquivo DBC | ✗ fail | 344ms | AssertionError: precisa de arquivo ST/SC para download: expected 0 > 0 |
...
| S10 | Sistema verifica CNES inválido sem crash | ✓ pass | 2ms |  |
| S11 | Pesquisador mapeia rede assistencial de SP por tipo | ✓ pass | 2ms |  |
