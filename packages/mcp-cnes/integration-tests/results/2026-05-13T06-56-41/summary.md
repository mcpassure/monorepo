...
| Q23 | equipamentosPorCnes HC FMUSP tem ressonância magnética (031) | ✓ pass | 1ms |  |
...
| Q36 | co_municipio de São Paulo é 355030 (6 dígitos) | ✓ pass | 1ms |  |
...
| F02 | buscarPorCodigoCnes: CNES inexistente retorna null | ✓ pass | 43ms |  |
| F03 | buscarPorNomeFallback: 'hospital das clinicas' retorna resultados | ✗ fail | 48ms | AssertionError: deve retornar ao menos 1 resultado: expected 0 > 0 |
| F04 | buscarPorNomeFallback: todos resultados têm source=online_fallback | ✗ fail | 51ms | AssertionError: assertGreater: expected 0 > 0 |
| F05 | buscarPorNomeFallback: codigoCnes tem 7 dígitos | ✗ fail | 43ms | AssertionError: assertGreater: expected 0 > 0 |
| F06 | buscarPorNomeFallback com UF filtra resultados | ✓ pass | 48ms |  |
| F07 | diagnóstico: formato codUnidade correto (7 vs 13 dígitos) | ✓ pass | 96ms |  |

### t04-ftp (6 passaram / 3 falharam / 9 total)
...
| FTP07 | converterDbcParaCsv: blast converte DBC para CSV | ✗ fail | 33ms | BlastNaoEncontradoError: Executável 'blast' não encontrado no PATH. Instale antes de executar a sincronização:   macOS:  |
| FTP08 | ingerirCsv: insere registros de ST SC no banco | ✗ fail | 1ms | TypeError: iconv.decodeStream is not a function |
| FTP09 | ingestão idempotente: re-ingerir mesmo CSV não duplica registros | ✗ fail | 0ms | TypeError: iconv.decodeStream is not a function |

### t05-scenarios (15 passaram / 0 falharam / 15 total)

| ID | Nome | Status | Duração | Detalhe |
|----|------|--------|---------|---------|
| S01 | Médico busca o Hospital das Clínicas de SP pelo nome | ✓ pass | 0ms |  |
| S02 | Gestor verifica dados cadastrais do HC FMUSP | ✓ pass | 1ms |  |
...
| S06 | Gestor em SC verifica hospitais de Florianópolis | ✓ pass | 0ms |  |
...
| S10 | Sistema verifica CNES inválido sem crash | ✓ pass | 1ms |  |
| S11 | Pesquisador mapeia rede assistencial de SP por tipo | ✓ pass | 0ms |  |
| S12 | Radioterapia: verificar equipamentos do INCA | ✓ pass | 1ms |  |
| S13 | Múltiplos hospitais em DF para comparação de leitos UTI | ✓ pass | 0ms |  |
